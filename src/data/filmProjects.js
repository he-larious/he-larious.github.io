const filmProjects = [
  {
    title: "Into the Shadows (Short Film)",
    description:
      "Credited as Social Media Editor. Produced fan-edit style promos (reels/TikToks) from film footage to align with dystopian themes and drive online engagement.",
    links: [
      { label: "Watch Film", url: "https://www.youtube.com/watch?v=dERotVauy5k" },
      { label: "Sample Edit", url: "https://www.instagram.com/p/C_qsZc9NIQX/?utm_source=ig_web_copy_link&igsh=MWt2Z3Zidnh5dDRk" }
    ]
  },
  {
    title: "Social Media Content Creator",
    description:
      "Edited 150+ fan edits that blended transitions and audio-visual alignment to enhance storytelling impact. Built a YouTube channel with 2.5K+ subscribers and 1M+ total views and expanded cross-platform reach (IG 850+, TikTok 100+).",
    links: [
      { label: "YouTube", url: "https://www.youtube.com/@helarious" },
      { label: "Instagram", url: "https://www.instagram.com/panikacake/" },
      { label: "TikTok", url: "https://www.tiktok.com/@he.larious" }
    ]
  }
];

export default filmProjects;
